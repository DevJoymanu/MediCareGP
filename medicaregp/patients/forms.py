...
class PatientForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            # Person responsible (Section 2)
            'responsible_surname':    forms.TextInput(attrs={'class': INPUT}),
            'responsible_first_name': forms.TextInput(attrs={'class': INPUT}),
            'responsible_title':      forms.Select(attrs={'class': SELECT, 'style': 'width:100%;'}),
            'responsible_id_number':  forms.TextInput(attrs={'class': INPUT, 'placeholder': 'I.D. Number'}),
            'responsible_email':      forms.EmailInput(attrs={'class': INPUT}),
            'postal_address':         forms.Textarea(attrs={'class': TEXTAREA, 'rows': 2}),
            'postal_code':            forms.TextInput(attrs={'class': INPUT, 'placeholder': 'Code'}),
            'address':                forms.Textarea(attrs={'class': TEXTAREA, 'rows': 2}),
            'residential_code':       forms.TextInput(attrs={'class': INPUT, 'placeholder': 'Code'}),
            'responsible_tel_h':      forms.TextInput(attrs={'class': INPUT, 'placeholder': 'Tel. (H)'}),
            'responsible_tel_w':      forms.TextInput(attrs={'class': INPUT, 'placeholder': 'Tel. (W)'}),
            'responsible_cell':       forms.TextInput(attrs={'class': INPUT, 'placeholder': 'Cell Number'}),
            ...
            'work_code':              forms.TextInput(attrs={'class': INPUT, 'placeholder': 'Code'}),
            # Next of kin
