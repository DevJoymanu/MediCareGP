...
- **TURN/ICE:** `appointments/turn.py` `build_ice_servers()` served fresh per call at `/video/<room>/ice/`. Precedence: metered.ca API (`METERED_DOMAIN`/`METERED_API_KEY`, free tier, recommended) → coturn HMAC (`TURN_URLS`+`TURN_STATIC_AUTH_SECRET`) → static creds → STUN-only. Staff diagnostic at `/video/turn-test/`.
...
### Partial / fragile
- **TURN is not configured by default** (STUN-only) → video is unreliable on mobile/firewalled networks until `METERED_*` or `TURN_*` env vars are set on Railway. Verify with `/video/turn-test/`.
